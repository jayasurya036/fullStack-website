<?php  
	...
	include('php/basices/connect.php');
	sec();
	$pageEnd=$_SESSION['pageNumber']*3;
	$pageStart=$pageEnd-3;
	?>
...
<body>
	...
	<div class="wrapper">
		<div class="header">
			<div class="headerAndNav">
				...
				<div class="navbar">
					<a class="navBarChild hide" href="php/post">new post</a>
	...
	$conn=dbConnect();
		...
		$sqlRea="SELECT * FROM reaction  where postId<='{$pageEnd}' && postId>'{$pageStart}'";

		$redData=$conn->query($sqlRea);
		$p=0;
		if($redData->num_rows>0)
		{
			while ($recAllData=$redData->fetch_array()) {
				$like[$p]=$recAllData['postLikes'];
				$disLike[$p]=$recAllData['postDIslikes'];
				$favorites[$p]=$recAllData['postFavorites'];
				$p++;
			}

		}
		
		$sql= "SELECT * FROM cachePost where postId<='{$pageEnd}' && postId>'{$pageStart}'";
...
?>
			...
			<div class="mainBar">
				...
				<?php
					$k=0;
		while($postData=$posts->fetch_array())
				{
					
					echo '<div class="post">';
									?>
									 <div class="deletePost">
										<div class="deletePostLine1"></div>
										<div class="deletePostLine1"></div>
									</div>
									<?php
									echo '<div class="data">';
													...
													<?php
														...
														$id=$postData['postID'];
														?>
														<form action="php/post/postPage.php">
														...
														<?php
									echo '</div>';
									echo '<div class="userOpenion">';
																echo '<div class="reaction star" >';
																			?>
																			<form action="php/reaction/react.php" method="get">
																				<input name="id" type="hidden" value="<?php echo $id?>">
																				<input name="reac" class="rea" type="submit" value="star">
																				<?php
																					echo $favorites[$k];
																				?>
																			</form>
																...
																echo '<div class="reaction share">';
																			?>
																				<form action="php/reaction/react.php" method="get">
																					<input name="id" type="hidden" value="<?php echo $id?>">
																					<input name="reac" class="rea" type="submit" value="like">
																					<?php
																						echo $like[$k];
																					?>
																				</form>
																...
																echo '<div class="reaction dislike">';
																				?>
																			<form action="php/reaction/react.php" method="get">
																				<input name="id" type="hidden" value="<?php echo $id?>">
																				<input name="reac" class="rea" type="submit" value="dislike">
																				<?php
																					echo $disLike[$k];
																				?>
																			</form>
																...
																echo '<div class="reaction share">';
																			?>
																				<form action="php/reaction/react.php" method="get">
																					<input name="id" type="hidden" value="<?php echo $id?>">
																					<input name="reac" class="rea" type="submit" value="share">
																					<?php
					...
					if($id)
					{
						...
						{ 
							echo '<div class="numbers">';
								...
								?>
								<form action="php/pageChanger.php" method="get">
								<?php 
